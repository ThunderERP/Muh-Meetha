...
const loginSchema = z.object({
  tenantSlug: z.string().min(1, 'Company ID is required').max(60)
  ...
  return (
    <div className="bg-white rounded-2xl shadow-modal p-8">
      <div className="mb-6">
        ...
        <p className="text-text-secondary text-sm mt-1">Enter your company ID to continue</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={cn('erp-input pl-9', errors.tenantSlug && 'border-danger')}
            />
          </div>
          {errors.tenantSlug && <p className="mt-1 text-xs text-danger">{errors.tenantSlug.message}</p>}
        </div>
        ...
        >
          {loading ? (<><Loader2 size={16} className="animate-spin" /> Signing in&hellip;</>) : 'Sign In'}
        </button>
      </form>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="mt-4 bg-surface-muted rounded-lg p-3 text-2xs text-text-muted text-center">
          Demo: company <span className="font-mono">demo-corp</span> &middot; admin@democorp.in / Admin@1234
        </div>
      )}
    </div>
  )
